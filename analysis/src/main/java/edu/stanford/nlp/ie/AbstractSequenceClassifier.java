...
public abstract class AbstractSequenceClassifier<IN extends CoreMap> implements Function<String, String> {
     ...
     * @param outputFormat    The format to put the output in: one of "slashTags", "xml", or
     *                        "inlineXML"
     ...
     */
    public List<ChineseStringUtils.SegToken> segmentString(String sentence) {
        return segmentString(sentence, defaultReaderAndWriter);
    }

    public List<ChineseStringUtils.SegToken> segmentString(String sentence, DocumentReaderAndWriter<IN> readerAndWriter) {
        ObjectBank<List<IN>> docs = makeObjectBankFromString(sentence, readerAndWriter);
        List<ChineseStringUtils.SegToken> sync = new ArrayList<ChineseStringUtils.SegToken>();
        for (List<IN> doc : docs) {
            classify(doc);
            ChineseStringUtils.combineSegmentedSentence((List<CoreLabel>) doc, flags, sync);
        }
        return sync;
    }
     ...
     *
     * @param in Input data addNEWLCWords do we add new lowercase words from this
     ...
     *
     * @param modelName The name of the model file. Iff it ends in .gz, then it is assumed
     *                  to be gzip compressed.
     * @param props     A Properties object which can override certain properties in the
