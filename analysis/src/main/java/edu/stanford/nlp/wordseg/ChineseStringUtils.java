...
public class ChineseStringUtils {
    ...
    public static final Pattern midDotRegex = Pattern.compile(ChineseUtils.MID_DOT_REGEX_STR + "+");

    public static boolean isLetterASCII(char c) {
        return c <= 127 && Character.isLetter(c);
    }

    private static BiFunction<CoreLabel, Integer, Integer> getOffsetter = (cwi, offset) -> {
        Integer startIdx = cwi.get(CoreAnnotations.CharacterOffsetBeginAnnotation.class);
        return startIdx == null ? offset : startIdx;
    };
    ...
    public static class SegToken {
        ...
        public SegToken(final String t) {
            term = t;
        }
    }


    public static void combineSegmentedSentence(
            List<CoreLabel> doc, SeqClassifierFlags flags, List<SegToken> sync) {
        ...
        for (Iterator<CoreLabel> wordIter = doc.iterator(); wordIter.hasNext();
            ...
            if (wi.get(CoreAnnotations.AnswerAnnotation.class).equals("1") && !("0".equals(String.valueOf(wi.get(CoreAnnotations.PositionAnnotation.class))))) {
                ...
                if (seg) {
                    String term = flags.sighanPostProcessing ? postProcessingAnswer(ans.toString(), flags) : ans.toString();
                    SegToken segToken = new SegToken(term);
                    segToken.charOffsetStart = lastStart;
                    lastStart = -1;
                    segToken.charOffsetEnd = getOffsetter.apply(pwi, testContentIdx) + 1;
                    sync.add(segToken);
            ...
            if (lastStart == -1) {
                lastStart = getOffsetter.apply(wi, testContentIdx);
            }
        ...
        if (ans.length() != 0) {
            ...
            SegToken segToken = new SegToken(term);
            segToken.charOffsetStart = lastStart;
            segToken.charOffsetEnd = getOffsetter.apply(wi, testContentIdx) + 1;
            sync.add(segToken);
            ans.delete(0, ans.length());
        }
    }
    ...
    static Character[] puncs = {'\u3001', '\u3002', '\u3003', '\u3008', '\u3009', '\u300a', '\u300b',
            '\u300c', '\u300d', '\u300e', '\u300f', '\u3010', '\u3011', '\u3014',
            '\u3015'};
    static Pattern[] puncsPat = new Pattern[puncs.length];
