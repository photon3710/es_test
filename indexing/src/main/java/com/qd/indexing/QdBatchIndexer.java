...
import com.qd.util.BoundedForkJoinPool;
import com.qd.util.OptionAccessor;
import com.qd.util.ValueAggregator;
import com.qd.util.iters.ChunkIterable;
...
/**
 * This is used to create indices in batch on elasticsearch.
 *
 * Created by xiaoyun on 4/18/14.
 */


public class QdBatchIndexer<T> {
	private static Logger LOGGER = LoggerFactory.getLogger(QdBatchIndexer.class);
    ...
    QdBatchIndexer(Node node, final Function<T, String> m, final Function<T, String> f) {
        mapper = m;
        this.node = node;
        idExtractor = f;
    }
    ...
    /**
     * This method does indexing in parallel, with specified batch size. 
     * The parallelism is determined by number of cpu core.
     * @param batchSize
     * @param avroStream
     */
